...
if __name__ == "__main__":
    ...
    req.request_id = "requestIdExample123456789"
    try:
        resp = client.check_user_amount(req)
        if resp.code == "0000":
            # 操作成功
            print("操作成功 ", resp.data)
        else:
    ...
    req.request_id = "requestIdExample123456789"
    try:
        resp = client.get_order_lxlw(req)
        if resp.code == "0000":
            # 操作成功
            print("操作成功 ", resp.data)
        else:
    ...
    # 查询劳动者退款订单信息
    req = GetLaborRefundOrderRequest(
        dealer_id = conf.dealer_id,
        order_id = "202009010016562012987",
        channel = "银行卡",
    )
    ...
    req.request_id = "requestIdExample123456789"
    try:
        resp = client.get_labor_refund_order(req)
        if resp.code == "0000":
            # 操作成功
            print("操作成功 ", resp.data)
        else:
    ...
    req.request_id = "requestIdExample123456789"
    try:
        resp = client.cancel_order_in_batch(req)
        if resp.code == "0000":
            # 操作成功
            print("操作成功 ", resp.data)
        else:
